...
export class AutoGenerator {
   ...
   */
  initialize(bootstrap: (context: BootstrapContext) => Promise<ApplicationRef>): void {
  ...
  private generateAllPages(bootstrap: (context: BootstrapContext) => Promise<ApplicationRef>): void {
  ...
  private async generatePagesForConfig(
    ...
    const fullUrl = `//${serverConfig.HOST}${pageConfig.path}`;

    // Pre-render the page
    const page = await renderWithSsr(bootstrap, fullUrl);
